...
class CollisionDetectionBehavior extends PhysicalBehavior {
  ...
  void onMount() {
    super.onMount();
    _hitboxProxy.overrideGame = parent.game;
  }
  ...
  void nonMapCollisionDetection(double dt) {
    ...
    // (kurtome): This should probably be changed to track "all collisions"
  ...
  void _calculateTilemapHits(bool Function(PhysicalEntity) filter) {
    ...
    // (kurtome): cache this somehow so it's only evaluated once per game
    //  loop?
    final nonMapCollidables = world.physicals.where(
...
class _HitboxProxyComponent extends PhysicalEntity {
  ...
  late LeapGame overrideGame;

  @override
  LeapGame get game => overrideGame;
}
